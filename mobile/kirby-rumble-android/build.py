#!/usr/bin/env python3
"""Build Kirby Brawler 2 for Android: a signed APK, and a zip to hand around.

    python3 build.py                  # the game as it is on origin/main
    python3 build.py --source FILE    # or any copy of kirby-rumble.html
    python3 build.py --debuggable     # test build that Chrome DevTools can attach to

The zip (dist/KirbyBrawler2-Android.zip) holds the APK, install notes, and the
...
def read_game(args):
    ...
    git = ['git', '-C', REPO]
    text = subprocess.run(git + ['show', args.ref + ':kirby-rumble.html'], check=True,
                          capture_output=True, encoding='utf-8').stdout
    last = subprocess.run(git + ['log', '-1', '--format=%h %cs', args.ref, '--', 'kirby-rumble.html'],
                          check=True, capture_output=True, encoding='utf-8').stdout.strip()
    return text, '%s:kirby-rumble.html (last changed %s)' % (args.ref, last)
...
def patch(html, what, old, new):
    ...
    if n != 1:
        die('cannot apply the "%s" patch: expected this exactly once in the game, found it %d times:\n    %s\n'
            'kirby-rumble.html has changed; update mobile_html() in build.py to match.' % (what, n, old[:140]))
...
def mobile_html(game, origin, version):
    ...
    html = patch(game, 'build stamp', '<!DOCTYPE html>\n',
                 '<!DOCTYPE html>\n<!-- %s mobile edition %s, built from %s by mobile/kirby-rumble-android/build.py -->\n'
                 % (APP_NAME, version, origin))
...
def build_apk(html, version_name, version_code, debuggable):
    ...
    apk = os.path.join(DIST, 'KirbyBrawler2-debug.apk' if debuggable else 'KirbyBrawler2.apk')
...
def package_zip(apk, html, version_name):
    ...
    out = os.path.join(DIST, 'KirbyBrawler2-Android.zip')
...
def main():
    ...
    ap.add_argument('--ref', default='origin/main', help='git ref to take kirby-rumble.html from (default: origin/main)')
    ap.add_argument('--source', help='build this kirby-rumble.html instead of the one at --ref')
