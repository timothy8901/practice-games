...
(() => {
  ...
  async function loadArt() {
    ...
    await Promise.all(jobs.map(async ([role, core]) => {
      ...
      const model = await KBModels.load(BASE + key + '.glb');
      art.geo[key] = shape(model, role, core);
      art.mat[key] = model.material;
    }));
    art.arena = await KBModels.load(BASE + 'arena.glb');
    art.ready = true;
  }

  // ------------------------------------------------------------- fighters
  const buildOriginal = window.buildKirby;
  window.buildKirby = function (palette, abilityKey) {
    ...
    // The Motes are limbless: no feet, no Kirby face, no ability hat.
  ...
  window.startFight = function (ability) {
    if (!art.ready && !art.failed) {
      ...
      if (card && !card.querySelector('.kb-loading')) {
        card.insertAdjacentHTML('beforeend', '<div class="overlay-sub kb-loading">Loading fighters…</div>');
      }
      return;
    }
  ...
  loadArt().then(() => {
    dressArena();
    console.log('[KB] Thrixel art ready');
    ...
    console.error('[KB] Thrixel art failed, keeping the built-in look: ' + err.message);
    art.failed = true;
