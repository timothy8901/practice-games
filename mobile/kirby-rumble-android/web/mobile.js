/* Kirby Brawler 2 - mobile layer. build.py injects this after the game's own script.
 *
 ...
 *   KB.back()     -> true if the game used the Back press (pause, resume, menus)
 *   KB.appPause() -> the app is about to leave the screen
 *
...
(() => {
  ...
  if (TOUCH) root.classList.add('kb-touch');
  if (IN_APP) root.classList.add('kb-app');
  ...
  const PILL = (label, color) => `<span class="kb-pill" style="--c:${color}">${label}</span>`;
  ...
  stickEl.className = 'kb-stick';
  const knob = document.createElement('div');
  knob.className = 'kb-knob';
  stickEl.append(knob);
  layer.append(stickEl);

  // Radii are at scale 1; layout() scales them to the screen.
  const btns = {};
  for (const [act, r, label, ring] of [['a1', 42, 'Attack', true], ['a2', 33, 'Special', true],
                                       ['shield', 32, 'Shield', true], ['jump', 32, 'Jump', false]]) {
    const el = document.createElement('div');
    el.className = 'kb-btn kb-' + act;
  ...
  const pauseBtn = mkTop('kb-pause', ICON.pause, 'Pause');
  const soundBtn = mkTop('kb-sound', ICON.sound, 'Sound on/off');
  wrap.append(topBar);
  ...
  window.renderOverlay = function () {
    ...
    if (!card) return;
    if (game.phase === 'menu') {
      card.insertAdjacentHTML('afterbegin', '<div class="kb-brand">Kirby Brawler 2</div>');
      const subs = card.querySelectorAll('.overlay-sub');
      if (TOUCH && subs[0]) {
        subs[0].innerHTML = 'One circular arena. You vs a CPU Kirby with a random ability.<br>' +
          `Drag the left side of the screen to move. Every ability has two attacks, ${PILL('Attack', COLORS.a1)} ` +
  ...
  function adaptResolution(t) {
    ...
    if (perf.cap > 1) {
      ...
      resize();
      console.log('[KB] ' + fps.toFixed(0) + ' fps: render scale down to ' + perf.cap + 'x');
    } else if (!perf.shadowsCut && typeof sun !== 'undefined' && sun.shadow) {
      perf.shadowsCut = true;
      sun.shadow.mapSize.set(1024, 1024);
      if (sun.shadow.map) { sun.shadow.map.dispose(); sun.shadow.map = null; }
      console.log('[KB] ' + fps.toFixed(0) + ' fps: shadow map down to 1024');
    }
  }
  ...
  function tick(t) {
    ...
    if (now !== fighting) {
      fighting = now;
      document.body.classList.toggle('kb-fighting', now);
      if (now) layout(); else releaseAll();
    }
    const p = game.player, c = game.cpu;
    if (!p) return;
    const A = ABIL[p.ability];
    if (p.ability !== lastAbility) {
      lastAbility = p.ability;
      btns.a1.label.textContent = A.a1.label;
      btns.a2.label.textContent = A.a2.label;
    }
  ...
  requestAnimationFrame(tick);
  console.log('[KB] mobile layer ready ' + JSON.stringify({
